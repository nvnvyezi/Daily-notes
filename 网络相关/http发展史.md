...
> 在http1.0中，每次请求都要建立一次单独的连接，如果需要长连接就必须手动设置connnection，而http1.1默认支持长连接，也就是不需要等到上一个请求的响应回来就可以在继续发起一个新的请求，但是要注意服务器返回响应必须按照发送的顺序，因此如果前面的请求一旦阻塞，后面的也会阻塞。http1.1还加入了只发送头部，比如说一个需要权限认证的请求，在头部发送过去不符合的情况下，就不会继续发送body部分，从而节省了带宽。而在验证通过的情况下，会返回一个100的状态码，表示可以继续进行发送，http1.1还加入了range头部，相对于http1.0来说如果我需要一个文件的部分东西，我就必须拿到这个文件。但是http1.1就可以规定一个范围，只需要返回这个范围内的东西以及状态吗206，因为虚拟站点技术的成熟，有可能一个站点对应多个域名，因此http1.1还加入了host域来进行区分，http1.1还加入了新的缓存机制，比如etag和cache-control，http1.0使用expries和last-modified和pragma，http1.0使用content-length来表示文件的大小以及判断文件是否传输结束，这对于一些动态传输的东西不太适应，http1.1引入了chunkedtransfer-coding，使用将发送消息分割成若干个任意大小的数据块，每个数据快都携带着这个数据块的大小，并且最后结束的话是发送一个零长度的数据块，并且最后还会会传输一个trailer，他包含一个或者多个头域，并且是在发送方在传输完所有快之后进行计算的。
>
> 在http1.1和2.0中间还有一个SPDY，他是google提出的，也是基于tcp的一个连接，并且他的提出不是为了替代http，而是加强http，用以提高网络加载速，提升用户体验，减小网络延迟，SPDY他加入了多路复用的技术，即多个请求流公用一个tcp连接，他解决了http1.1 HOL（排头阻塞）的问题，不会因为前面的阻塞而整体阻塞，而多路复用带来的问题就是我请求优先级高的请求有可能不会被立即处理，这就引入了SPDY新加入的另一个东西，请求优先级 ，SPDY还加入了头部压缩。因为前面http的请求的时候一些头部可能是没有用的或者重复多余的，因此采用头部压缩是很有必要的。SPDY还加入了服务端推送，比如说在请求一个html文件的时候，里面如果还有css和js文件，以前的话我又必须再继续发起请求，但是SPDY就会主动将这两个文件推回去，减少了请求数。并且SPDY也可以采用HTTPS的连接方式，提高传输安全性。
>
